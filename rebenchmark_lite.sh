...
NUM_RUNS=20
COOLDOWN_SEC=10
# LITE: skip known-bad cells (SSD ncnn + ShuffleNet tflite)
skip_cell() {
    case "$1:$2" in
        # FIXED: ssd ncnn now works via trimmed raw3 model (was segfault)
        # ssd_mobilenet_v2:ncnn_cpu|ssd_mobilenet_v2:ncnn_vulkan) return 0 ;;
    ...
    for mode in "${POWER_MODES[@]}"; do
        ...
        for model in "${CLASSIFICATION_MODELS[@]}"; do
            for rt in "${ALL_RUNTIMES[@]}"; do
                if skip_cell "$model" "$rt"; then echo "  [LITE-skip] $model/$rt"; continue; fi
