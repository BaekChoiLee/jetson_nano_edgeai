...
echo "=== Local built onnxruntime-gpu 설치 ==="

LOCAL_WHEEL=$(ls ~/onnx/onnxruntime/build/Linux/Release/dist/onnxruntime_gpu-*.whl 2>/dev/null | head -n 1)

if [ -z "$LOCAL_WHEEL" ]; then
    echo "[ERROR] 로컬 빌드 wheel을 찾을 수 없음"
    echo "확인 경로: ~/onnx/onnxruntime/build/Linux/Release/dist/"
    exit 1
fi

echo "설치할 wheel: $LOCAL_WHEEL"

python3 -m pip uninstall -y onnxruntime onnxruntime-gpu || true

python3 -m pip install --user --force-reinstall "$LOCAL_WHEEL"
